...
{  
    ...
    class ShellSorter
    {
        ...
        private int[] GetStepSeries (int N)
        {
            ...
            int first = 0;
            int last = 1;
            int[] stepSeries = new int[N/2];
            int i = stepSeries.Length - 1;
            bool ok = true;
            while ((i >= 0) && ok)
            {
                sum = last + first;
        ...
        public void SortForCartoon (int[] array, ListBox listBoxForNums, ListBox listBoxForSelectedNums, ListBox listBoxForSwapNums)
        {
            NumberOfCompares = 0;
            NumberOfSwaps = 0;
            int[] stepSeries = GetStepSeries(array.Length); // последовательность шагов для заданной размерности массива
            int t = 0;
            while (stepSeries[t] == 0)
            {
                t = t + 1;
            }
            int countSteps = stepSeries.Length; // вычисляем количество шагов    
            for (int j = t; j < countSteps; j++)
            {    
